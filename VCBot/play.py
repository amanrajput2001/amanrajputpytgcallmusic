 ...
 if GRPPLAY or (m.from_user and m.from_user.is_contact) or m.outgoing:
   ...
   if replied:
      if replied.audio or replied.voice:
         huehue = await replied.reply("`Download Ho rha... thoda wait krle bc ✨❤️`")
         ...
         if chat_id in QUEUE:
          ...
          try:
            ...
            await huehue.edit(f"**Song chal gya  party de ab bc✨❤️ ▶** \n**🎧 SONG** : [{songname}]({link}) \n**💬 CHAT** : `{chat_id}`", disable_web_page_preview=True)
         ...
         if len(m.command) < 2:
            ...
            huehue = await m.reply("`Song milne tak wait kro    yaaaa  bhad  me jao ✨❤️...`")
         ...
         if len(m.command) < 2:
            ...
            huehue = await m.reply("`song milne tak wait kro  ya bhad me jao✨❤️...`")
            query = m.text.split(None, 1)[1]
            search = ytsearch(query)
            if search==0:
               await huehue.edit("`Found Nothing for the Given Query`")
               ...
               if hm==0:
                  ...
                  if chat_id in QUEUE:
                     ...
                     try:
                        ...
                        add_to_queue(chat_id, songname, ytlink, url, "Audio", 0)
                        await huehue.edit(f"**Song chalne lga   ab party de bc✨❤️ ▶** \n**🎧 SONG** : [{songname}]({url}) \n**💬 CHAT** : `{chat_id}`", disable_web_page_preview=True)
                     except Exception as ep:
                        await huehue.edit(f"`{ep}`")
      ...
      if hm==0:
         ...
         if chat_id in QUEUE:
            ...
            try:
               ...
               await huehue.edit(f"Song chal gya  ab party de bc✨❤️ **[Radio 📻]({link})** in `{chat_id}`", disable_web_page_preview=True)
            except Exception as ep:
               await huehue.edit(f"`{ep}`")
